...
CSX             csx             CV
CU
CU1             cu1             CP          Replaced by CPhase as of qiskit 0.16.0
...
from sympy.abc import lamda as sym_lamda

from ..operations import StdGate, StdCtrlGate, OperatorStructure
from ..states import Qubit, Variable
from .stdgates_2q import XX, YY, ZZ
from .stdgates_1q import Rx, Ry, Rz

# from .stdgates_forest import CPhase

__all__ = ("U3", "U2", "CRz", "CRx", "CRy", "CRz", "Rxx", "Ryy", "Rzz")
...
class U3(StdGate):
    ...
    cv_sym_operator = sym.Matrix([
            [sym.cos(sym_theta / 2), -sym.sin(sym_theta / 2) * sym.exp(sym.I * sym_lamda)],
            [
                sym.sin(sym_theta / 2) * sym.exp(sym.I * sym_phi),
                sym.cos(sym_theta / 2) * sym.exp(sym.I * (sym_phi + sym_lamda)),
            ],
        ])

    def __init__(
        self, theta: Variable, phi: Variable, lamda: Variable, q0: Qubit
    ) -> None:
        super().__init__(theta, phi, lamda, q0)

    @property
    def H(self) -> "U3":
        return U3(-self.theta, -self.lamda, -self.phi, *self.qubits)
...
class U2(StdGate):
    ...
    cv_sym_operator = U3.cv_sym_operator.subs({sym_theta: sym.pi/2})

    def __init__(self, phi: Variable, lamda: Variable, q0: Qubit) -> None:
        super().__init__(phi, lamda, q0)

    @property
    def H(self) -> "U3":
        return U3(-sym.pi / 2, -self.lamda, -self.phi, *self.qubits)
...
#         q1: Qubit,
#     ) -> None:
#         super().__init__(theta, phi, lamda, q0, q1)
...
#     #             CNot(0, 1),
#     #             U3(-theta / 2, 0.0, -(phi + lambda_) / 2, 1),
#     #             CNot(0, 1),
...
#     @property
#     def H(self) -> "CU3":
#         return CU3(-self.theta, -self.lamda, -self.phi, *self.qubits)

#     def _diagram_labels_(self) -> List[str]:
#         return [CTRL, "U3({theta}, {phi}, {lambda_})"]
...
class CRx(StdCtrlGate):
    ...
    def __pow__(self, t: Variable) -> "CRx":
        return CRx(self.theta * t, *self.qubits)
...
class CRy(StdCtrlGate):
    ...
    def __pow__(self, t: Variable) -> "CRy":
        return CRy(self.theta * t, *self.qubits)
...
class CRz(StdCtrlGate):
    ...
    def __pow__(self, t: Variable) -> "CRz":
        return CRz(self.theta * t, *self.qubits)
...
class Rxx(StdGate):
    ...
    """
    cv_sym_operator = XX.cv_sym_operator.subs(sym_t, sym_theta/sym.pi)
    ...
    def __pow__(self, t: Variable) -> "Rxx":
        return Rxx(self.theta * t, *self.qubits)
...
class Ryy(StdGate):
    ...
    Same as YY(theta/pi)
    """
    cv_sym_operator = YY.cv_sym_operator.subs(sym_t, sym_theta/sym.pi)
    ...
    def __pow__(self, t: Variable) -> "Ryy":
        return Ryy(self.theta * t, *self.qubits)
...
class Rzz(StdGate):
    ...
    Same as ZZ(theta/pi).
    """
    cv_sym_operator = ZZ.cv_sym_operator.subs(sym_t, sym_theta/sym.pi)
    ...
    def __pow__(self, t: Variable) -> "Rzz":
        return Rzz(self.theta * t, *self.qubits)
